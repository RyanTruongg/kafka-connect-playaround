package com.leap.jconsumer.web.rest;
...
import com.leap.jconsumer.config.KafkaProperties;
...
class JconsumerKafkaResourceIT {
    ...
    void setup() {
        ...
        JconsumerKafkaResource kafkaResource = new JconsumerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jconsumer-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jconsumer-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
